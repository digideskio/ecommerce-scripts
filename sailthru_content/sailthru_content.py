...
def main(argv):
    ...
    parser.add_argument(
            '--oauth_host',
            default='https://courses.stage.edx.org/oauth2',
    ...
    if args.command == 'list':
        while True:
            ...
            if not response.is_ok():
                logger.error("Error code %d connecting to Sailthru content api: %s", response.json['error'], response.json['errormsg'])
                return
    ...
    client = EdxRestApiClient('https://stage-edx-discovery.edx.org/api/v1/', oauth_access_token=access_token)
    ...
    while page:
        ...
        # if not response.:
        #     logger.error("Error reading from content discovery API")
        #     return
            ...
            for course_run in course['course_runs']:
                ...
                if sailthru_content:
                    ...
                    if not response.is_ok():
                        logger.error("Error code %d connecting to Sailthru content api: %s", response.json['error'], response.json['errormsg'])
                        return
                    # if course_runs > 40: return
                    # logger.info(response.json)


    logger.info('Retrieved %d courses.', count)
    logger.info('Saved %d course runs.', course_runs)
...
def convert_tag(type, tag):
    ...
    """
    if tag:
        resp = type + '-' + tag.replace(' & ', '-').replace(',','').replace('.','').replace(' ', '-').replace('--','-').casefold()
        return resp
    return ''
